...
bool ImGui::Tab ( const char* label, bool selected )
{
    ...
    const ImRect rect ( pos, ImVec2 ( pos.x + 95, pos.y + 18 ) );
    ...
    if ( selected )
        it_anim->second.active_text_anim += 15 * ( 1.f - ImGui::GetIO ( ).DeltaTime );
    else
        it_anim->second.active_text_anim = 0;

    if ( it_anim->second.active_text_anim > 90 )
        it_anim->second.active_text_anim = 90;
    else if ( it_anim->second.active_text_anim < 0 )
        it_anim->second.active_text_anim = 0;

    if ( selected )
        it_anim->second.active_gradient_alpha += 15 * ( 1.f - ImGui::GetIO ( ).DeltaTime );
    else
        it_anim->second.active_gradient_alpha = 0;

    if ( it_anim->second.active_gradient_alpha > 100 )
        it_anim->second.active_gradient_alpha = 100;
    else if ( it_anim->second.active_gradient_alpha < 0 )
        it_anim->second.active_gradient_alpha = 0;

    if ( selected )
        it_anim->second.x_text_anim += 3 * ( 1.f - ImGui::GetIO ( ).DeltaTime );
    else
        it_anim->second.x_text_anim -= 3 * ( 1.f - ImGui::GetIO ( ).DeltaTime );

    if ( it_anim->second.x_text_anim > 10 )
        it_anim->second.x_text_anim = 10;
    else if ( it_anim->second.x_text_anim < 0 )
        it_anim->second.x_text_anim = 0;

    window->DrawList->AddRectFilledMultiColor ( ImVec2 ( rect.Min.x + ( it_anim->second.x_text_anim + 5 ), rect.Min.y ), rect.Max,
        ImColor ( 0, 60, 170, it_anim->second.active_gradient_alpha ),
        ImColor ( 0, 60, 170, 0 ), ImColor ( 0, 60, 170, 0 ),
        ImColor ( 0, 60, 170, it_anim->second.active_gradient_alpha ) );
    window->DrawList->AddText ( ImVec2 ( ( rect.Min.x + 8 + it_anim->second.x_text_anim ), ( rect.Min.y + rect.Max.y ) / 2.f - ( label_size.y / 2.f ) ), ImColor ( 255, 255, 255, 210 ), label );
    window->DrawList->AddText ( ImVec2 ( ( rect.Min.x + 8 + it_anim->second.x_text_anim ), ( rect.Min.y + rect.Max.y ) / 2.f - ( label_size.y / 2.f ) ), ImColor ( 0, 60, 170, it_anim->second.active_text_anim ), label );
...
void ImGui::Hotkey ( const char* str, int* k )
{
    ImGuiWindow* window = GetCurrentWindow ( );
    if ( window->SkipItems )
        return;
...
bool ImGui::Checkbox ( const char* label, bool* v )
{
    ...
    const float square_sz = 8;
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb ( pos, pos + ImVec2 ( square_sz + 12 + label_size.x, ( label_size.y > square_sz ) ? label_size.y - 5 : square_sz ) );
    ItemSize ( total_bb, style.FramePadding.y );
    ...
    static std::map<ImGuiID, int> alpha_table;
    if ( alpha_table.find ( id ) == alpha_table.end ( ) )
        alpha_table.emplace ( id, 122 );

    auto& alpha = alpha_table[ id ];

    if ( *v && alpha < 255 )       alpha += 20 + ImGui::GetIO ( ).DeltaTime;
    else if ( !*v && alpha > 0 )   alpha -= 20 + ImGui::GetIO ( ).DeltaTime;

    window->DrawList->AddRectFilled ( total_bb.Min, total_bb.Min + ImVec2 ( square_sz, square_sz ), ImColor ( 35, 35, 35 ) );

    window->DrawList->AddRectFilledMultiColor ( total_bb.Min, total_bb.Min + ImVec2 ( square_sz, square_sz ),
        ImColor ( 51, 0, 255, alpha ),
        ImColor ( 51, 0, 255, alpha ),
        ImColor ( 93, 0, 255, alpha ),
        ImColor ( 93, 0, 255, alpha ) );

    window->DrawList->AddRect ( total_bb.Min, total_bb.Min + ImVec2 ( square_sz, square_sz ), ImColor ( 255, 255, 255, 15 ) );

    window->DrawList->AddText ( ImVec2 ( total_bb.Max.x - label_size.x - 5, total_bb.Min.y - 4 ), ImColor ( 255, 255, 255, alpha + 175 ), label );
...
bool ImGui::BeginCombo2 ( const char* label, const char* preview_value, ImGuiComboFlags flags )
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow ( );
...
bool ImGui::Combo2 ( const char* label, int* current_item, bool ( *items_getter )( void*, int, const char** ), void* data, int items_count, int popup_max_height_in_items )
{
    ImGuiContext& g = *GImGui;
...
bool ImGui::Combo2 ( const char* label, int* current_item, const char* const items[ ], int items_count, int height_in_items )
{
    const bool value_changed = Combo2 ( label, current_item, Items_ArrayGetter, ( void* )items, items_count, height_in_items );
    return value_changed;
}
...
bool ImGui::SliderScalar ( const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags )
{
    ...
    const ImVec2 label_size = CalcTextSize ( label, NULL, true );
    const ImRect total_bb ( window->DC.CursorPos + ImVec2 ( 5, 0 ), window->DC.CursorPos + ImVec2 ( w, label_size.y + 11 ) );
    const ImRect frame_bb ( total_bb.Min + ImVec2 ( -3, label_size.y + 6 ), total_bb.Max );
    ...
    window->DrawList->AddRectFilled ( frame_bb.Min, frame_bb.Max, ImColor ( 35, 35, 35 ) );
    window->DrawList->AddRectFilledMultiColor ( frame_bb.Min, { grab_bb.Min.x + 3, frame_bb.Max.y },
        ImColor ( 51, 0, 255 ),
        ImColor ( 51, 0, 255 ),
        ImColor ( 93, 0, 255 ),
        ImColor ( 93, 0, 255 ) );
    window->DrawList->AddRect ( frame_bb.Min, frame_bb.Max, ImColor ( 255, 255, 255, 15 ) );

    RenderText ( total_bb.Min - ImVec2 ( 3, 0 ), label );
...
bool ImGui::Selectable ( const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg )
{
    ...
    window->DrawList->AddText ( text_min + ImVec2 ( 0, 1 ), hovered ? ImColor ( 255, 255, 255, 150 ) : selected ? ImColor ( 111, 0, 255, 255 ) : ImColor ( 255, 255, 255, 100 ), label );
